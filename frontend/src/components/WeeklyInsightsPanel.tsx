...
interface WeeklyInsightsPanelProps {
  ...
  /** Full scan count before client-side filters (for "X of Y" display). */
...
export default function WeeklyInsightsPanel({
  ...
  const insights = useMemo(() => {
    ...
    for (const stock of matches) {
      ...
      totalYoy += stock.price_yoy_pct;
      total4w += stock.price_qoq_pct;
      ...
      const retail = stock.retail_holding_pct;
    ...
    return {
      ...
      topIndustries,
      avgPriceYoy: totalYoy / matches.length,
      avgPrice4w: total4w / matches.length,
  ...
  return (
    <div className="golden-insights weekly-insights">
      <h3 className="golden-insights-title">📊 Weekly Portfolio Insights</h3>
      <div className="golden-insights-grid">
        <div className="golden-insight-card">
          ...
          <span className="golden-insight-value">
            {insights.totalStocks}
            {totalUnfiltered != null && totalUnfiltered !== insights.totalStocks
              ? ` of ${totalUnfiltered}`
              : ""}
          </span>
        </div>
        ...
        <div className="golden-insight-card">
          <span className="golden-insight-label">Avg Weekly YoY</span>
          <span className="golden-insight-value positive">
            +{insights.avgPriceYoy.toFixed(1)}%
          </span>
        </div>

        <div className="golden-insight-card">
          <span className="golden-insight-label">Avg 4-Week</span>
          <span className="golden-insight-value positive">
            +{insights.avgPrice4w.toFixed(1)}%
          </span>
        </div>
        ...
        <div className="golden-insight-card">
          ...
          <span className="golden-insight-value">
            {insights.highPromoter} (
            {((insights.highPromoter / insights.totalStocks) * 100).toFixed(0)}%)
          </span>
        </div>
        ...
        <div className="golden-insight-card">
          ...
          <span className="golden-insight-value">
            {insights.lowRetail} (
            {((insights.lowRetail / insights.totalStocks) * 100).toFixed(0)}%)
          </span>
        </div>
      </div>
