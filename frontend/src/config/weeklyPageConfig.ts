...
import type { SortOption, FilterValues } from "../lib/filterSort";
import type { WeeklyStockMatch } from "../types/weekly";

export const WEEKLY_SORT_OPTIONS: SortOption<WeeklyStockMatch>[] = [
  { id: "rank", label: "Rank", getValue: (m) => m.rank_score ?? 0, defaultDirection: "desc" },
  { id: "price", label: "Price", getValue: (m) => m.price },
  { id: "priceGrowth", label: "YoY", getValue: (m) => m.price_yoy_pct, defaultDirection: "desc" },
  { id: "price4w", label: "4W", getValue: (m) => m.price_qoq_pct, defaultDirection: "desc" },
  {
    id: "revenueGrowth",
    label: "Revenue",
...
export const WEEKLY_FILTER_SECTIONS: FilterSectionDef[] = [
  {
...
export const WEEKLY_ADVANCED_SECTION_TITLES = ["Advanced"];
...
export function buildWeeklyIndustryOptions(
  matches: WeeklyStockMatch[],
